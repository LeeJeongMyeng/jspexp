...
--%>
	...
	<table id="tb">
		...
		%>
		<tr><td><input type="hidden" name="proc" value=""/><input type="checkbox" name="cidx" value="<%=idx%>"/></td>
...
/*
  #요청값을 통한 쿠키의 생성/수정 삭제처리
  	1. 파일정보
  		1) a05_CookieLiist.jsp : 쿠키값 전체 리스트
  		2) a09_proc.jsp : 생성/수정/삭제 따라서 변경처리..
  	2. 개발순서
  		1) 쿠키값 전체 리스트 화면
  			- 기능 버튼 처리
  				등록/수정/삭제
  			- form action="a09_proc.jsp"
  			- 기능 요청값 처리 : <input type="hidden" name="proc"/>
  			- 버튼을 눌렀을 때, script 처리
  			 	function go(proc){
  					document.querySelector("[name=proc]").value=proc
  				}
