...
<%--
# fmt유형  jstl
1. 데이터 유형을 변경하거나, 원하는 데이터를 처리할 떄, 주로 활용 된다.
2. 날짜 처리(java.util.Date)
	1) 날짜+시간
	2) 기본 형식
	  <fmt:formatDate type="date|time|both"
	  	dateStlye="full|short"
	  	fimeStyle="full|short"
	  	pattren="yyyy/MM/dd kk:mm:ss:.S"
	  	value="선언된 날짜 형 데이터 />
3. 숫자 처리
	1) 숫자를 통화나 소숫점이하 처리, 지정된 형식으로 처리할 떄 활용된다.
	2) type의 속성
		currency:통화단위
		percent:%처리
	3) pattern : 00,000,000.000 등과 같이 특정한 출력형식을 처리할 수 있다.

 --%>
<%
 request.setAttribute("now", new Date());//오늘 날짜 객체 생성
 request.setAttribute("num01", 7899345459.75);//숫자데이터 생성
%>
<h2>날짜 format형식</h2>
<table>
	<col width="30%">
	...
	<tr><th>시간+날짜(기본형식)</th><td><fmt:formatDate value="${now }" type="both"/></td></tr>
	<tr><th>패턴형식(기본형식)</th><td><fmt:formatDate value="${now }" pattern="yyyy/MM/dd kk:mm:ss:S"/></td></tr>
</table>
...
<table>
	...
	<tr><th>숫자패턴</th><td><fmt:formatNumber pattern="###,###,###,###" value="${num01 }"/></td></tr>
</table>
</body>
<script>
/*
