...
<%--
	#pageContext 기본 객체
		...
		3. 기능 메서드
			...
			getRespones(): response기본 객체를 가져온다.
			getSession(): session 기본 객체를 가져온다.
			getSerbletContext(): application 객체를 가져온다.
			getServletConfig(): config 기본 객체를 가져온다.
			getOut() : out기본 객체를 가져온다.
			getException(): exception 기본객체를 가져온다.
			getPage() : page기본객체를가져온다
--%>
<%
 //페이지 범위 데이터 설정.
 //자바의 최상위 객체인 Object형식으로 저장이 된다.
 //Object srt01 = "안녕하세요"; 이것을 str01이란 이름으로 저장
 // 문자열인 경우 Object객체의 toString()으로 저장된 문자열을 처리하기에 출력이 가능하다.
 // 그 외 다른 유형(숫자 등..)은 typeCastring을 선언하여야한다.
 ...
 Object ob01 = new Person("마길동",25,"서울"); //상위는 하위할당 가능(저장)
...
<h1>페이지범위 숫자연산:<%=((Integer)pageContext.getAttribute("num01")+ 
		...
		<%-- setAttribute로 설정되는 내용은 기본적으로  object로 저장되기에 고유의 유형(숫자,boolean) --%>
