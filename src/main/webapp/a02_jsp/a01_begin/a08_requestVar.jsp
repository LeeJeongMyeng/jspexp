...
<%--
	...
	#요청값이 없을때, default 데이터를 위한 처리
	  1. if조건을 통해서 null인경우
	     1) 문자열 ==>""
	     2) 숫자 ==> 0
	     초기화면에서 form요소 객체에 의해서 요청값을 전송하지 않기 때문에
	     null 데이터인 경우로 처리된다.
	     ==> 한페이지에서 form입력화면과 요청값을 전달한 결과 데이터 출력화면이 
...
<%--학생명, 국어,영어,수학 점수를 요청값을 처리하고 default값으로 "없음" 
 ? :초기화면 -request.getParameter("sname") ==null
 ?sname=&kor=&eng=&math=
 	:입력하지 않고 submit입력했을 때
 		-request.getParameter("sname").equals("")
 		ps)서버에 전송되기 전에 js단에서 form요소가 입려고디게 처리한다.
...
// sname=홍길동&kor=100&eng=90&math=70
// 실수형인 경우 Double.parseDouble("26.7")
